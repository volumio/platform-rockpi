...
then
  ...
  ls "${C}"/kernel-config/master/linux-"${K}"-"${B}".config
  cp "${C}"/kernel-config/master/linux-"${K}"-"${B}".config "${A}"/userpatches/
fi
...
if [ -e "${A}/output/debs/linux-headers-${B}-${K}_${ARMBIAN_VERSION}"* ]
then
